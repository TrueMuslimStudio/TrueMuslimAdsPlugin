...
public class TrueAdLimitUtils {
    ...
    public static boolean isBanned(Context context, String unitId, String adType) {
        ...
        if (clicksCount >= clicksLimit || impressionsCount >= impressionsLimit) {
            TruePrefUtils.getInstance().init(context, unitId).resetClicksCounter();
            TruePrefUtils.getInstance().init(context, unitId).resetImpressionsCounter();
            TruePrefUtils.getInstance().init(context, unitId).updateBanTime();
            banned = true;
        }
        return banned;
